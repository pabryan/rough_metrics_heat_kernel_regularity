...
\title{Heat kernels and regularity for rough metrics on smooth manifolds}
...
Using this lemma, the Harnack inequality can be deduced relatively easily from the 
Harnack inequality stated in \cite{SC}.
...
\begin{proof}
First of all, Lemma \ref{eq:soln1} below shows that a solution $u$ of the $\Delta_{g,\cW}$ heat equation is a weak solution, hence Moser's parabolic Harnack inequality \cite{MR0159139,MR0288405} is expected to apply. The only reason we cannot immediately apply Moser's result is because of the factor $a_x$. But this factor is time independent, measurable and $a_x > 0$ almost everywhere, which is precisely the situation described in \cite[Section 4]{SC}. Thus, on noting that the results in \cite{SC} localise, we can apply Theorem 5.3 in \cite{SC} which is precisely our Harnack inequality stated in the theorem.
\end{proof}
...
We note that this theorem can also be obtained, albeit for dimensions $\geq 3$
and with a lot more work, from the  results in \cite{CS}. There, the authors establish
such Harnack estimates for weak solutions allowing for the coefficients to degenerate 
in a controlled way via methods of real-variable harmonic analysis.
In order to simplify our exposition, we instead allude to the local 
nature of \cite{SC} as we have done. 
...
\begin{rem}
Note that by this result, we obtain that $\hk^g_{t}: M \times M \to R_+$ is at least continuous. 
Hence, the a priori separable measurability of the heat kernel that we established earlier 
immediately improves to joint measurability via this result. 
\end{rem}
