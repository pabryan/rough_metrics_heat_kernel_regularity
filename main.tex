...
and show that either our notion of a funciton is sufficiently weak to capture the classic shit
or it is necessary for us to formulate the problem in this way because
we don't have the classical methods at hand}
\end{rem}

\section{The heat kernel and its regularity}
...
5.2.1 in \cite{Davies}. This proof was adapated for
the setting of rough metrics in the setting that $M$ is
compact in \cite{BCont} and therefore, we just provide
a sketch of the construction. The fundamental estimate we
require is the following \emph{Harnack inequality}. 

\begin{itemize}
\item[] At each $x \in M$ and $t > 0$, there exists a precompact
open set $U_x$, a constant $C = C(t, U_x)  > 0$ such that
\begin{equation}
\tag{H}
\label{eq:Harnack}
\sup_{(y,s) \in U_x \times (\frac{1}{2}t,\frac{3}{2}t)} u(y,s) 
	\leq \inf_{(y,s) \in U_x \times (2t,\frac{5}{2}t)} u(y,s)
\end{equation}
\end{itemize}

The passage from this to the construction (and regularity) of $\hk^{g,\cW}_t$ is as follows.

\begin{prop}
Suppose that \eqref{eq:Harnack} holds. Then, the heat
kernel $\hk^{g,\cW}_{\mdot}: \R_{+} \times M \times M \to \R$
exists and for each precompact open $K \subset M$
and $t \in [t_1, t_2]$ with $0 <t_1 < t_2$, 
there exists an $\alpha$ such that $\hk^{g,\cW}_t \in \Ck{\alpha}(M \times M)$.
\end{prop} 
\begin{proof}
\begin{enumerate}[(i)]
...
\item Now, left $f \in \Lp{2}(M)$, and write
	...
	we have that $u(x,t) = e^{-t \Delta_{g,\cW}} f = u_+(x,t) - u_-(x,t)$.
...
\item Since $u_{\pm}(x,t) \geq 0$ are positive solutions,
	...
	\modulus{e^{-t \Delta_{g,\cW}}f(x)}  &= \modulus{u(x,t)} = u_+(x,t) + u_-(x,t) \\
		&\leq C(t,U_x) (u_+(y,2t) + u_-(y,2t)) \\
		&= C(t,U_x) \modulus{u(y,2t)}
		= C(t,U_x)  \modulus{e^{-2t \Delta_{g,\cW}}f(y)}
	\end{align*} 
	for each $y \in U_x$.
	...
	\modulus{e^{-t \Delta_{g,\cW}}f(x)} 
		&\leq \frac{C(t,U_x))}{\omega_g(U_x)} \int_{U_x} \modulus{e^{-2t \Delta_{g,\cW}}f(y)}\ d\omega_g(y) \\
		&\leq \frac{C(t,U_x)}{\omega_g(U_x)^{\frac{1}{2}}} \norm{e^{-2t \Delta_{g,\cW}f}},
	\end{align*}
...
\item	Again on using that $e^{-t\Delta_{g,\cW}}$ is positive
	preserving, $a_{t,x} \geq 0$ and  therefore $\hk^{g,\cW}_t(x,y) \geq 0$.
	Then, the semigroup property again shows that $y \mapsto \hk^{g,\cW}_t(x,y)$
	is a non-negative solution to the heat equation and therefore,
	we can apply \eqref{eq:Harnack} to obtain
	oscillation estimates which yeild the local $\Ck{\alpha}$
	regularity in the conclusion (c.f. \cite{}).
\end{enumerate} 
\end{proof}
...
In this section, we demonstrate that the Harnack 
estimate \eqref{eq:Harnack} holds for the rough 
metrics we consider. We do this via a localisation
procedure to pull the problem into $\R^n$
and then alluding to results for weighted Sobolev
