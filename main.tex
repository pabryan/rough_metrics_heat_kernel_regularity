...
For $t > 0$, $0 < \kappa < \tau$ and an $0 < \epsilon < \kappa$, define
\begin{equation}
\label{eq:paradef}
\begin{split} 
Q_{(x,t)}(\kappa, \tau,\epsilon) &= \set{(y,s) \in V_x \times (\kappa,\tau): \modulus{t - s} < \epsilon^2,\ d_x(x,y) < 2 \epsilon} \\
...
Q^+_{(x,t)}(\kappa, \tau, \epsilon) &= \set{(y,s) \in V_x \times (0,T): s \in \cbrac{t + \frac{3}{4}\epsilon^2, t + \epsilon^2},\ 
	d_x(x,y) < \frac{1}{2} \epsilon}
\end{split}
\end{equation}
...
\note{Figure out exactly how large $\epsilon$ can be}
\end{thm} 
