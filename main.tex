...
measurable map $\hk^{g,\cW}_{\mdot}: \R_+ \times M \times M \to \R$
is a heat kernel if 
$$u(t,x) = \int_{M} \hk^{g,\cW}_t(x,y) u_0(y)\ d\omega_g(y),$$
with $\lim_{t \to 0} \hk^{g,\cW}(\mdot,y) \to \delta(y)$, 
...
a sketch of the construction. The fundamental estimate in 
need of proof to obtain existence is the following: 

\begin{enumerate}
\item[(F)\label{Def:F}]
For $x \in M$ and $t > 0$, there exists
$C_{t,x} \geq 0$ such that
$$\modulus{e^{-t\Delta_{g,\cW}}f(x)} \lesssim C_{t,x} \norm{f}_2.$$ 
\end{enumerate}

The passage from this to the construction of $\hk^{g,\cW}_t$ is as follows.

\begin{prop}
Suppose that \ref{Def:F} holds. Then, the heat
