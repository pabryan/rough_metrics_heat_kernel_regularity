...
induced by such metrics. We demonstrate that globally continuous heat kernels exist with local
Hölder continuity in space and time. This is done by Riesz representation methods, and employing
local Harnack estimates for weak solutions of divergence form elliptic equations with bounded
...
is more general than that of \cite{Davies} and we need to obtain a certain 
...
open set $U_x$, a $\delta_t > 0$ and a constant $C = C(t, U_x)  > 0$ such that
\begin{equation}
\tag{H}
\label{eq:Harnack}
\esssup_{y \in U_x} u(y,t) 
	\leq C \essinf_{y \in U_x} u(y, t + \delta_t)
\end{equation}
\end{itemize}
